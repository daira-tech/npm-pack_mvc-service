import { TAggregateFuncType, TColumnInfo, TKeyFormat, TNestedCondition, TOperator, TQuery, TSelectExpression, TSortKeyword } from "./Type";
import { IDbClient } from './IDbClient';
import ValidateValueUtil from './ValidateValueUtil';
import { PgSelectExpression } from './PgSqlUtils/PgSelectExpression';
import { PgWhereExpression } from './PgSqlUtils/PgWhereExpression';
import PgUpdateExpression from './PgSqlUtils/PgUpdateExpression';
import { BaseTableModel } from './BaseTableModel';

export class PgTableModel extends BaseTableModel {
    ...
    private get createSqlFromJoinWhere(): string {
        ...
        for (const join of this.joinConditions) {
            ...
            const query = PgWhereExpression.createCondition(join.conditions, this, this.vars.length + 1);
    ...
    public async find<T = {[key: string]: any}>(
        ...
        if (selectColumns == "*") {
            for (const key of Object.keys(this.Columns)) {
                selects.push(PgSelectExpression.create({model: this, name: key}, null, null, keyFormat));
            }
        ...
        if (typeof pkOrId === 'string' || typeof pkOrId === 'number' || typeof pkOrId === 'boolean') {
            ...
            query = PgWhereExpression.createConditionPk(this, {id: pkOrId});    
        } else {
            query = PgWhereExpression.createConditionPk(this, pkOrId);
        }
    ...
    public select(param1: Array<string | {name: string, alias?: string, func?: TAggregateFuncType}> | "*" | string = "*", param2?: BaseTableModel | string | TKeyFormat, param3?: TKeyFormat) {
        if (param1 === "*") {
            ...
            for (const key of Object.keys(model.Columns)) {
                this.selectExpressions.push(PgSelectExpression.create({model: model, name: key}, null, null, keyFormat));
            }
            return;
        }
        ...
        if (Array.isArray(param1)) {
            ...
            for (const key of param1) {
                if (typeof key === 'string') {
                    ...
                    this.selectExpressions.push(PgSelectExpression.create({model: model, name: key.name}, key.func ?? null, key.alias ?? null, keyFormat));
                }
            }
            return;
        }
    ...
    public selectNullToEmptyString(column: string | {name: string, model: BaseTableModel}, alias: string) {
        ...
        this.selectExpressions.push(`${PgSelectExpression.nullToEmptyString(column)} as "${alias}"`);
    }
    ...
    public selectDateAsFormat(column: string | {name: string, model: BaseTableModel}, to: 'date' | 'time' | 'datetime', alias: string) {
        ...
        this.selectExpressions.push(`${PgSelectExpression.createDateTime(column, to)} as "${alias}"`);
    }
    ...
    public where(param1: string | TColumnInfo | Array<TNestedCondition>, param2?: Array<any> | TOperator, right?: TColumnInfo | any): void {
        if (typeof param1 === 'string') {
            if (param2 === undefined || right === undefined || Array.isArray(param2)) {
                ...
                const query = PgWhereExpression.create({model: this, name: param1}, param2, right, this.vars.length + 1);
        ...
        if ('model' in param1 && 'name' in param1) {
            if (param2 === undefined || right === undefined || Array.isArray(param2)) {
                ...
                const query = PgWhereExpression.create(param1, param2, right, this.vars.length + 1);
                this.whereExpressions.push(query.expression);
                if (query.vars !== undefined) {
                    this.vars = [...this.vars, ...query.vars];
                }
        ...
        if (Array.isArray(param1)) {
            const query = PgWhereExpression.createCondition(param1, this, this.vars.length + 1);
            this.whereExpressions.push(query.expression);
            if (query.vars !== undefined) {
                this.vars = [...this.vars, ...query.vars];
            }
    ...
    public async update(pkOrId: string | number | boolean | {[key: string]: string | number | boolean}, options: {[key: string]: any}) : Promise<void> {
        ...
        const updateSetQuery = PgUpdateExpression.createUpdateSet(this, options);
        let whereQuery: TQuery;
        if (typeof pkOrId === 'string' || typeof pkOrId === 'number' || typeof pkOrId === 'boolean') {
            ValidateValueUtil.validateId(this.Columns, pkOrId);
            whereQuery = PgWhereExpression.createConditionPk(this, {id: pkOrId}, updateSetQuery.vars);    
        } else {
            whereQuery = PgWhereExpression.createConditionPk(this, pkOrId, updateSetQuery.vars);
        }
    ...
    public async delete(pkOrId: string | number | boolean | {[key: string]: any}) : Promise<void> {
        ...
        if (typeof pkOrId === 'string' || typeof pkOrId === 'number' || typeof pkOrId === 'boolean') {
            ValidateValueUtil.validateId(this.Columns, pkOrId);
            whereQuery = PgWhereExpression.createConditionPk(this, {id: pkOrId});    
        } else {
            whereQuery = PgWhereExpression.createConditionPk(this, pkOrId);
        }
